...
function generateBehaviorJsonArray(name, txt) {
  let isExist = Object.prototype.hasOwnProperty.call(
    window.parameters,
    "token"
  );
  if (isExist) {
    if (name != "houseAppreciation") {
      ...
      if (window.addHouseBrowsingArray.length > 0) {
        ...
        for (let i = 0; i < window.addHouseBrowsingArray.length; i++) {
          ...
          for (let j = 0; j < jsonArray.length; j++) {
            if (
              window.addHouseBrowsingArray[i].houseId == jsonArray[j].houseId
            ) {
              bl = true;
              jsonArray[j].num = jsonArray[j].num - 0 + 1;
              jsonArray[j].browsLength =
                jsonArray[j].browsLength -
                0 +
                (window.addHouseBrowsingArray[i].browsLength - 0);
            }
          }
          if (!bl) {
      ...
      if (window.addHouseBrowsingArray.length > 0) {
        window.addHouseBrowsingArray[
          window.addHouseBrowsingArray.length - 1
        ].endTime = new Date() - 0;
        window.addHouseBrowsingArray[
          window.addHouseBrowsingArray.length - 1
        ].browsLength = (
    ...
    if (window.behaviorJsonArray.length > 0) {
      if (
        name !=
        window.behaviorJsonArray[window.behaviorJsonArray.length - 1].name
      ) {
        window.behaviorJsonArray[window.behaviorJsonArray.length - 1].endTime =
          new Date() - 0;
        window.behaviorJsonArray[
          window.behaviorJsonArray.length - 1
        ].seconds = (
          (new Date() -
            0 -
            window.behaviorJsonArray[window.behaviorJsonArray.length - 1]
              .beginTime) /
          1000
        ).toFixed(0);
        //发送日志
  ...
  switch (to.name) {
    ...
    case "hardcoverRoaming":
      window.addHouseBrowsingArray.push({
        houseId:
          window.bottomData[to.query.bottomDataName][to.query.bottomDataIndex]
            .maps[to.query.id].id,
        houseType:
          window.bottomData[to.query.bottomDataName][to.query.bottomDataIndex]
            .maps[to.query.id].house_type,
        num: 1,
        browsLength: 0,
        beginTime: new Date() - 0,
        endTime: 0,
    ...
    case "mainPage":
      if (!window.indexInfo) {
        if (!window.parameters.token || window.parameters.token == "null") {
          //获取首页数据（未授权）
          let isMinProgram = 1;
