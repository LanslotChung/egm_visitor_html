...
<script type="text/javascript" src="https://webapi.amap.com/maps?v=1.1&key=9b6d113ef3c481a23b2f27fcb471f5e8&plugin=AMap.Driving"></script>
<script>
  ...
  methods: {
    doLoction() {
      ...
      if (window.parameters.token) {
        ...
        // 根据起终点经纬度规划驾车导航路线
        this.driving.search(
          new AMap.LngLat(window.parameters.longitude, window.parameters.latitude),
       ...
       // let appPath="androidamap://viewMap?sourceApplication="+this.productObj.name+"&poiname="+this.productObj.name+"&lat="+this.productObj.latitude+"&lon="+this.productObj.longitude+"&dev=0";
        let path='https://uri.amap.com/marker?position=' + this.productObj.longitude + ',' + this.productObj.latitude+'&name=' + this.productObj.name
